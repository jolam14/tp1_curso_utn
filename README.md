...
CSS3: Para la estilización visual (uso de variables :root, Flexbox, Grid, bordes redondeados, interacciones :hover).
...
Cabecera (.bar): Se utilizó un contenedor principal para agrupar el logo de la empresa y el título principal (<h1>) a la izquierda mediante un sub-contenedor, separándolos del botón de configuración de tema situado a la derecha mediante Flexbox.
...
La estilización se llevó a cabo utilizando un enfoque modular basado en maquetación flexible y precisa:
...
Alineación de color (Tarjeta Violeta): La IA generaba errores al intentar extraer el diseño específico con el color violeta. Para mantener la alineación visual y la coherencia del diseño, fue necesario adaptar y utilizar un patrón de otro color. A modo de registro, las imágenes originales sin editar se adjuntaron en la raíz del proyecto, fuera de la carpeta de assets, para documentar el estado previo a los ajustes.

Personalización de los Checkboxes: Los inputs nativos de tipo checkbox poseen estilos predeterminados por el navegador que rompían con el diseño oscuro. Se resolvió aplicando appearance: none para ocultar el cuadro por defecto y se construyó un diseño personalizado simulando el tilde ("palomita") mediante el pseudo-elemento ::after.
